...
class MallRVAdapter(private val calDiff: Boolean = false) : RecyclerView.Adapter<MallRVAdapter.MCommonViewHolder>(), IRVAdapter {

    companion object {
        const val TAG = "MallRVAdapter"
    }

    private val list = mutableListOf<Any>()

    val delegate: MallRVDelegate

    init {
        delegate = MallRVDelegate(object : IDataAdapter {
    ...
    override fun setItems(items: List<Any>) {
        if (calDiff && list.size > 0) {
            val result = DiffUtil.calculateDiff(RvDiffCallback(list, items))
            list.clear()
            list.addAll(items)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MCommonViewHolder {
        return MCommonViewHolder(delegate.createView(parent, viewType)).also {
    ...
    override fun onBindViewHolder(holder: MCommonViewHolder, position: Int) {
        ...
        delegate.bindView(holder.itemView, model, position, this)
    }
