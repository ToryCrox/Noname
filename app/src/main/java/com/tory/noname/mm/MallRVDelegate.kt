...
import com.tory.noname.mm.IRVAdapter
import com.tory.noname.mm.IView
import com.tory.noname.mm.MLogger
 ...
 */
class MallRVDelegate(private val dataAdapter: IDataAdapter) {

    private val viewTypes = mutableListOf<ViewType<*>>()
    private val viewTypeMap = ArrayMap<Class<*>, ViewType<*>>()
    private val asyncData = ArrayMap<Class<*>, List<Any>>()
     ...
     * 类型注册
     */
    inline fun <reified V, reified M : Any> register(viewClz: Class<V>) where V : IView<M>, V : View {
        val clazzType = M::class.java
        checkRegister(clazzType)
        registerGroupType(null, clazzType)
        val viewType = ViewType(clazzType) { parent -> createView(viewClz, parent) }
    ...
    ) where V : IView<M>, V : View {
        val clazzType = M::class.java
        checkRegister(clazzType)
        registerGroupType(groupType, clazzType)
        val margin = transformMargin(groupMargin)
        val viewType = ViewType(clazzType, gridSize = gridSize, 
            poolSize = poolSize, margin = margin) { parent -> creator(parent) }
        addViewType(viewType)
    }
    ...
    fun <V : View> createView(clazz: Class<V>, parent: ViewGroup):
        ...
        return try {
            ...
            if (isDebug) {
                throw RuntimeException("createView Error can not create View clazz:$clazz")
            } else {
                MLogger.e( "createView Error", e)
                View(parent.context)
            }
        }
    }
    ...
    fun getViewTypeIndex(position: Int): Int {
        ...
        if (viewType < 1 && isDebug) {
            throw IllegalArgumentException("getItemViewType can not found view type for model: $model," +
                " please check you register the Model")
        }
        return viewType
    }
    ...
    private fun getViewTypeByIndex(viewType: Int): ViewType<*>? {
        ...
        if (type == null && isDebug) {
            throw RuntimeException("can not found viewType: viewType:$viewType")
        }
        return type
    }
    ...
    fun createView(parent: ViewGroup, viewType: Int): View {
        ...
        if (isDebug) {
            ...
            MLogger.d("$TAG createView viewType:$viewType, " +
     ...
     * 绑定数据
     */
    fun bindView(view: View, item: Any, position: Int, rvAdapter: IRVAdapter) {
        if (view !is IView<*>) {
            return
        }
        ...
        (view as IView<Any>).update(item)
        if (isDebug) {
            val timeSpent = SystemClock.elapsedRealtime() - startTime
            MLogger.d("$TAG bindView position:$position groupPosition:${view.groupPosition}" +
