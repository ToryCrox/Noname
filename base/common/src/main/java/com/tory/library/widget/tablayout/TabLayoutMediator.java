...
/**
 * A mediator to link a MTabLayout with a ViewPager2. The mediator will synchronize the ViewPager2's
 * position with the selected tab when a tab is selected, and the MTabLayout's scroll position when
 * the user drags the ViewPager2. TabLayoutMediator will listen to ViewPager2's OnPageChangeCallback
 * to adjust tab when ViewPager2 moves. TabLayoutMediator listens to MTabLayout's
 ...
 * the mediator object, {@link #attach()} will link the MTabLayout and the ViewPager2 together. When
 ...
 * {@link #attach()} call. Changing the ViewPager2 or MTabLayout will require a new instantiation of
 * TabLayoutMediator.
 */
public final class TabLayoutMediator {
    ...
    @Nullable
    private MTabLayout.OnTabSelectedListener onTabSelectedListener;
    @Nullable
    ...
    /**
     * Link the MTabLayout and the ViewPager2 together. Must be called after ViewPager2 has an adapter
    ...
    /**
     * Unlink the MTabLayout and the ViewPager. To be called on a stale TabLayoutMediator if a new one
    ...
    void populateTabsFromPagerAdapter() {
        ...
        if (adapter != null) {
            ...
            for (int i = 0; i < adapterCount; i++) {
                MTabLayout.Tab tab = tabLayout.newTab();
     ...
     *
     * <p>This class stores the provided MTabLayout weakly, meaning that you can use {@link
    ...
    private static class TabLayoutOnPageChangeCallback extends ViewPager2.OnPageChangeCallback {
        ...
        TabLayoutOnPageChangeCallback(MTabLayout tabLayout) {
        ...
        public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
            MTabLayout tabLayout = tabLayoutRef.get();
            if (tabLayout != null) {
     ...
     */
    private static class ViewPagerOnTabSelectedListener implements MTabLayout.OnTabSelectedListener {
        private final ViewPager2 viewPager;
